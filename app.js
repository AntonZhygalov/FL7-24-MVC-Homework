...
var sortingView = {
    ...
    render: function () {
        for (var i = 0; i < model.allPersons.length; i++) {
            $('.sortings').append(`<li><span class="up"></span><span class="down"></span></li>`);
        }
    },
    handleClicks: function () {
        $('.sortings').on('click', 'li', function (e) {
            var $currentLi = $(e.target.parentNode);
            var currentIndex = $currentLi.index();
            if ($(e.target).is('.up') && currentIndex != 0) {
                let person = model.allPersons[currentIndex - 1];
                model.allPersons[currentIndex - 1] = model.allPersons[currentIndex];
                model.allPersons[currentIndex] = person;
            } else if ($(e.target).is('.down') && currentIndex != (model.allPersons.length - 1)) {
                let person = model.allPersons[currentIndex + 1];
                model.allPersons[currentIndex + 1] = model.allPersons[currentIndex];
                model.allPersons[currentIndex] = person;
            }
...
var briefInfo = {
    ...
    render: function () {
        $('p').html(`Selected person is <b>${control.getCurrentPerson().name}.</b> Person's score is: ${control.getCurrentPerson().score}`);
    }
}
